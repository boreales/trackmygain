...
if ! docker inspect "$SRC" >/dev/null 2>&1; then
    echo "✗ No container matches '$SRC'." >&2
...
if [ ! -f .env ]; then
    echo "✗ No .env at $ROOT — required to know POSTGRES_DB / POSTGRES_USER." >&2
...
# ─── 4. Locate destination volume (for sanity print only) ───────────────────
...
echo
echo "⚠  DESTRUCTIVE: the current contents of '$DEST_VOLUME' will be replaced"
...
if [ ! -s "$DUMP_FILE" ]; then
    echo "✗ Dump is empty. Aborting before wipe." >&2
    exit 1
fi
...
if [ "$ready" -ne 1 ]; then
    echo
    echo "✗ DB never became ready. Check: docker compose logs db" >&2
    exit 1
fi
...
# ─── 10. Bring everything else up; Flyway rattrape ──────────────────────────
